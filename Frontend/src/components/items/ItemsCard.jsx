...
export default function ItemCard({ data }) {
  ...
  const updateDB = async (newStatus, id) => {
    try {
      const response = await axios.put(`http://localhost:8081/updateAvailable/${id}`, {
        available: newStatus,
      });
  ...
  const ItemDelete = async (id) => {
    try {
      const response = await axios.delete(`http://localhost:8081/delete_item/${id}`);
      ...
      <div className="flex flex-col items-start gap-2 p-2 ">
        ...
        <div className="flex w-full ">
          <div className="flex items-center justify-between w-full">
            <button
              onClick={handleClick}
              className={`px-5 py-1 text-xs font-semibold text-white rounded border-[1px] ${buttonText === 'AVAILABLE' ? 'bg-green-600' : 'bg-red-600'}`}
            >
              {buttonText}
            </button>
            ...
            <div className="flex items-center w-auto h-auto justify-center gap-x-4">
              ...
              <button
                onClick={() => ItemDelete(data.itemID)}
              >
