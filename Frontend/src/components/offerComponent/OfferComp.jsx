...
export const OfferComp = (props) => {
	...
	useEffect(() => {
		const fetchData = async () => {
			try {
				const res = await axios.get(`${API_URL}/showoffer`,{withCredentials:true});
	...
	const OfferDelete = async (id) => {
		...
		try {
			const response = await axios.delete(`${API_URL}/delete_offer/${id},`,{withCredentials:true});
					...
					{bannerShow.length > 0 ? (
                      ...
                      {showEye[data.offerID] === "enable" ? (
                        ...
                        <FaRegEyeSlash className="text-red-500" size={24} />
										...
										<div
											className="bg-red-500 rounded-md p-1 hover:bg-red-700 active:scale-95"
											onClick={() => OfferDelete(data.offerID)}>
