...
function AnalysisItemToday() {
  ...
  useEffect(() => {
    axios.get('http://localhost:8081/month')
    .then(res => {
      setRawData(res.data);
  ...
  return (
      <div className='w-full h-screen border'>
          ...
          <div className='flex items-center felx-row xl:mt-1'>
            ...
            <Link to="/app/analysis/monthtime"><button className='px-12 py-2 text-sm font-semibold text-[#007FA8] bg-white rounded border-[#007FA8] border-[1px]'>Time</button></Link>
          </div>
          <div className='flex justify-end mx-2'>
            <button onClick={downloadChart} className='px-2 py-2 mx-3 text-black bg-white border-2 rounded-xl md:px-3 md:py-3 hover:bg-black hover:text-white'><IoMdDownload /></button>
          </div>
